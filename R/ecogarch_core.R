...
ecogarch_qmle <- function(Gdt, dt, init = list(a1 = 0.1, theta = -0.1, gamma = 0.3, mu = -3),
  ...
  lower <- c(log(1e-6), -Inf, -Inf, -Inf)
  upper <- c(log(10), Inf, Inf, Inf)
  fit <- if (grad == "fd") {
    stats::optim(par0, qll, gr = gr_fd, method = "L-BFGS-B", lower = lower, upper = upper)
  } else {
    stats::optim(par0, qll, method = "L-BFGS-B", lower = lower, upper = upper)
  }
  ff <- forward(fit$par)
  list(
    par = list(a1 = ff$a1, theta = ff$theta, gamma = ff$gamma, mu = ff$mu, lambda = lam_hat, K = K_hat),
