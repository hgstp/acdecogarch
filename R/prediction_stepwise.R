...
prediction_stepwise <- function(tQML, G, alpha = 0.05, start_k = 100, method = "approx", grad = "none") {
  ...
  G_upper <- rep(NA_real_, N)
  G_lower <- rep(NA_real_, N)
  ...
  inside <- function(seg) {
    if (length(seg)) sum(G[seg] <= G_upper[seg] & G[seg] >= G_lower[seg]) else NA_integer_
  }
  coverage <- c(inside(seg1), inside(seg2), inside(seg3))
  pct <- c(
    if (length(seg1)) coverage[1] / length(seg1) else NA_real_,
...
prediction_interval_coverage <- function(G, G_upper, G_lower, start_idx) {
  idx <- seq.int(start_idx, length(G))
  L <- length(idx)
  thirds <- floor(L / 3)
  seg1 <- idx[1:thirds]
  seg2 <- idx[(thirds + 1):(2 * thirds)]
  seg3 <- idx[(2 * thirds + 1):L]
  inside <- function(seg) {
    if (length(seg)) sum(G[seg] <= G_upper[seg] & G[seg] >= G_lower[seg]) else NA_integer_
  }
