...
#' @export
prev_tick <- function(t, x, X) {
  idx <- findInterval(t, x, left.open = FALSE)
  X[pmax(idx, 1)]
}
...
vol_prediction_density_piecewise <- function(Xn, dt_next, par, grid = NULL, exact = TRUE) {
  ...
  z_m <- (log(grid) - mu - varphi) / (theta + gamma)
  z_n <- (log(grid) - mu - varphi) / (theta - gamma)
  fZ <- function(z) sqrt(lam / (2 * pi)) * exp(-0.5 * lam * z^2)
  dens_m <- (1 / grid) * fZ(z_m) * (1 / abs(theta + gamma))
  dens_n <- (1 / grid) * fZ(z_n) * (1 / abs(theta - gamma))
  m_idx <- which(log(grid) - mu < varphi)
  n_idx <- which(log(grid) - mu >= varphi)
  dens <- numeric(length(grid))
  if (theta < -gamma && -gamma < 0) {
    dens[m_idx] <- dens_m[m_idx]
    dens[n_idx] <- dens_n[n_idx]
  } else if (-gamma < theta && theta < 0) {
    dens[m_idx] <- 0
    dens[n_idx] <- dens_m[n_idx] + dens_n[n_idx]
  } else if (0 < gamma && gamma < theta) {
    dens[m_idx] <- dens_n[m_idx]
    dens[n_idx] <- dens_m[n_idx]
  } else if (0 < theta && theta < gamma) {
    dens[m_idx] <- 0
    dens[n_idx] <- dens_m[n_idx] + dens_n[n_idx]
  } else {
    dens[m_idx] <- dens_m[m_idx]
  ...
  w <- c(diff(grid), tail(diff(grid), 1))
  total <- sum(dens * w)
  if (total > .Machine$double.eps) dens <- dens / total
  list(s = grid, density = dens, varphi = varphi)
}

#' Density mode
#' @export
mode_of_density <- function(s, d) {
  s[which.max(d)]
}
...
predict_one_step <- function(G, tQML, par, level = 0.95) {
  ...
  dt_next <- tail(dt, 1)
  dens <- vol_prediction_density_piecewise(Xn = X, dt_next = dt_next, par = par)
  s2_next <- mode_of_density(dens$s, dens$density)
