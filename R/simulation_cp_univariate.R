...
simulate_ecogarch11_cp_univariate <- function(par, lambda = 1, nmax = 3000, nu = 6, equidistant_step = NULL) {
  ...
  if (is.null(equidistant_step)) {
    return(list(G = G, t_obs = t_jump, vol = exp(mu + X)))
  }
  t_grid <- seq(equidistant_step, max(t_jump), by = equidistant_step)
  all_times <- sorted <- sort(c(t_jump, t_grid))
  Z_all <- rep(0, length(all_times))
  Z_all[match(t_jump, all_times)] <- Z
  s2_all <- numeric(length(all_times))
  X_all <- numeric(length(all_times))
  last_t <- 0
  Xcur <- 0
